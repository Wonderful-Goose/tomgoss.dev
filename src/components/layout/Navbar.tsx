    ...
    <header className="sticky top-0 z-10 bg-white dark:bg-neutral-900 border-b border-neutral-200 dark:border-neutral-800">
      <nav className="container mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <button className="md:hidden p-2" onClick={toggleMenu} aria-label="Toggle menu">
          {/* Hamburger icon */}
          <span
            className={`block w-6 h-0.5 bg-neutral-800 dark:bg-neutral-200 mb-1.5 transition-transform ${isMenuOpen ? 'rotate-45 translate-y-2' : ''}`}
          ></span>
          <span
            className={`block w-6 h-0.5 bg-neutral-800 dark:bg-neutral-200 mb-1.5 transition-opacity ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`}
          ></span>
          <span
            className={`block w-6 h-0.5 bg-neutral-800 dark:bg-neutral-200 transition-transform ${isMenuOpen ? '-rotate-45 -translate-y-2' : ''}`}
          ></span>
        </button>
      </nav>
